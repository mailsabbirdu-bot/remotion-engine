# 🚀 Revideo Overlay Engine - Colab Runner

```python
# @title 🎬 START REVIDEO OVERLAY RENDER
...
LOCAL_ROOT = "/content/revideo-production"
PROJECT_NAME = "revideo_project"
...
def setup_and_render():
    ...
    # Priority 2: Marker file match (master_revideo.json)
    if not project_dir:
        print_progress(2, 30, "Project folder name not found. Searching for marker file...")
        for root, dirs, files in os.walk("/content"):
            if "master_revideo.json" in files and "render-headless.js" in files:
                project_dir = root
                break
    ...
    # Sync Manifest
    drive_manifest = os.path.join(BASE_DRIVE, "manifests/master_revideo.json")
    local_manifest = os.path.join(project_dir, "master_revideo.json")
    ...
    final_destination_root = os.path.join(BASE_DRIVE, "renders/overlays/revideo")
